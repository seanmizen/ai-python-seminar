...
class GridPoint:
    ...
    def __init__(self, parent, x, y, max_occupants=1, occupants=None, label=None, N=None, S=None, E=None, W=None):
        ...
        if occupants is not None:
            while self._occupants < self._maxOccupants:
                self.placeOccupant(parent, occupants.pop(0))
    ...
    @property
    def neighbours(self):
        return [(neighbour.x, neighbour.y) for neighbour in self._neighbours if neighbour is not None and neighbour.capacity > 0]
    ...
    def vacate(self, occupant, direction=None):
        if occupant not in self._occupants:
            print("Occupant list for square({0},{1}): {2}".format(
                self.x, self.y, [occupant for occupant in self._occupants]))
            print("No such occupant: {0}:{1}".format(
                occupant, occupant.objectName))
            return None
        ...
        if self._neighbours[direction].occupied:
            return self
        self._occupants.remove(occupant)
        occupied = self._neighbours[direction].occupy(
            occupant, (self.x, self.y))
        if occupied is None:
            return self
        return occupied
...
class GridWorld:
    ...
    def __init__(self, h, w, max_time=0, update_interval=0.05, points=None, occupants=None):
        ...
        if points is None:
            self._grid = [[GridPoint(self, x, y)
                           for x in range(w)] for y in range(h)]
        elif occupants is None:
            self._grid = [[GridPoint(self, x, y, points[(x, y)])
                           for x in range(w)] for y in range(h)]
        else:
            self._grid = [[GridPoint(self, x, y, points[(x, y)], occupants[(
    ...
    def _tick(self):
        ...
        for agent in self._agents.values():
            action = agent[2].chooseAction(
                self, agent[0], agent[1], self._grid[agent[1]][agent[0]].occupants)
    ...
    def _applyAction(self, action: Action):
        ...
        if action.actionCode == action.inaction:
            return None
        ...
        if action.actionCode == action.move:
            ...
            newSquare = self._grid[action.y][action.x].vacate(
                action.agent, action.actionDirection)
        ...
        if action.actionCode == action.tag:
            # can only tag a location the agent is actually at
            if self._agents[action.agent.objectID][0] == action.x and self._agents[action.agent.objectID][1] == action.y:
                # and only if the target is actually there
                if action.actedUpon in self._grid[action.y][action.x].occupants and action.actedUpon.objectName == "target":
                    action.actedUpon.tagTarget()
                    return action.actedUpon
            # a failure case will not return the tagged target to the user.
            return None

    # gives out a map of the accessible nodes - which could include 'isolated' spots
    # from where nowhere else is reachable.
    def exportMap(self):
        return dict([((point.x, point.y),
                      dict([(neighbour, 1)
                            for neighbour in point.neighbours]))
                     for point in [p for row in self._grid for
                     p in row]
                     if point.capacity > 0])
    ...
    # this places some object at a location. If the object is an agent,
    # it also exports the map and any targets to it.
    def placeOccupant(self, occupant, row=-1, col=-1, visitor=False):
        if occupant.inWorld is None:
            occupant.embed(self)
        if row < 0 and occupant.y >= 0:
            row = occupant.y
        if col < 0 and occupant.x >= 0:
            col = occupant.x
        if self._grid[row][col].placeOccupant(self, occupant):
            if occupant.objectName == "agent":
                self._agents[occupant.objectID] = [col, row, occupant]
                # agents now receive a map
                occupant.importMap(self.exportMap())
                # and any targets that might have been set up
                for row in self._grid:
                    for point in (p for p in row if p.capacity > 0):
                        for existing in (e for e in point.occupants if e.objectName == "target"):
                            occupant.addGoalPoint(
                                existing.x, existing.y, existing.sequenceNum)
            return True
        return False
