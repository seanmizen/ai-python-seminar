...
def runWorld(sizeX, sizeY, time, stop, worldInfo):
    ...
    if 'targets' in worldInfo:
       ...
       for target in worldInfo['targets']:
           targetX = (gridagents.GridTarget(name="targetX",world=world,x=target[0],y=target[1],seq=seq))
...
# close to a wall.
worldValues1 = {'time': 0, 'agent1Pos': (round(worldX/2), round(worldY/2)), 'points': points1}
...
if targetsOn: worldValues['targets'] = targets
...
for target in worldValues['targets']:
    pygame.draw.circle(gridSquares[target[0]][target[1]],pygame.Color(0,128,128),(round(surfaceSize[0]/(2*worldX)),round(surfaceSize[1]/(2*worldY))),round(surfaceSize[0]/(4*worldX)))
displayedBackground.blits([(gridSquares[target[0]][target[1]],squares[target[0]][target[1]]) for target in worldValues['targets']])
      ...
      try:
          ...
          if curTime != worldValues['time']:
             ...
             for target in worldValues['targets']:
                 if target[2]:
                    redraw = True
                    pygame.draw.circle(gridSquares[target[0]][target[1]],pygame.Color(128,128,128),(round(surfaceSize[0]/(2*worldX)),round(surfaceSize[1]/(2*worldY))),round(surfaceSize[0]/(4*worldX)))
                    displayedBackground.blit(gridSquares[target[0]][target[1]],squares[target[0]][target[1]])
